...
def run_csp_galaxev(isedname, outname, sfh, sfh_pars, tau_V, mu, epsilon, work_dir, input_tmpname='tmp.in', output_tmpname='mySSP'):
    ...
    if sfh == 'tau':
        ...
        inputlines.append('6\n')
        inputlines.append(str(sfh_pars)+'\n')
    else:
        raise ValueError("Only 'tau','custom' and 'SSP' SFHs are currently implemented")
    ...
    os.system('scp %s/%s.ised %s/%s.ised'%(work_dir, output_tmpname, work_dir, outname))
    os.system('scp %s/%s.4color %s/%s.mass'%(work_dir, output_tmpname, work_dir, outname))
...
#def run_csp_galaxev(isedname, outname, sfh='tau', sfh_pars=1., tau_V=0.1, mu=0.3, epsilon=0., work_dir='./', input_tmpname='tmp.in', output_tmpname='mySSP'):
def run_csp_galaxev_no_dust(isedname, outname, sfh, sfh_pars, tau_V, mu, epsilon, work_dir, input_tmpname='tmp.in', output_tmpname='mySSP'):
