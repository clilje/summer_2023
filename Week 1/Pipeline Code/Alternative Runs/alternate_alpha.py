  ...
  for ppp in np.arange(len(line_wavelength)):
    ...
    for xx in range(len(logUs)):
        if logUs[xx] == round(log_Us*2)/2 and alpha[xx]== a and xid[xx] == x_id and nh[xx] == n_h:
            ...
            #print(line_name)
            #print(line_name[ppp][xx])
            #print(bh_lum)
            line_height=line_name[ppp][xx]/1e45*(10**bh_lum)
            #line_height=line_name[ppp][xx]/1e45*(10**bh_lum)[0]
                    #print(line_flux)
...
def add_spectrum(i, age_gal_i,bh_mass,bh_lum,met_gal,av_sfr,x_id,n_h,log_Us,a):
    ...
    age_row = [0]
    #read in row 1
    ages = pd.read_csv('norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'_BH_moredust_case2.ised_ASCII',delimiter='/s', header=None, skiprows = lambda x: x not in age_row)
    ...
    specific_rows = [6]
    #read in row 7
    wavelength = pd.read_csv('norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'_BH_moredust_case2.ised_ASCII', header=None,delimiter='/s', skiprows = lambda x: x not in specific_rows)
    ...
    df = pd.read_csv('norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'_BH_moredust_case2.ised_ASCII',delimiter='/s', header=None, skiprows=7, skipfooter=12)
    ...
    #get first 7 lines
    front = pd.read_csv('norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'_BH_moredust_case2.ised_ASCII',delimiter='/s', header=None, skiprows = lambda x: x not in  [0,1,2,3,4,5,6])
    
    ###this section gets the "footer" section of the file in its undisturbed form, to join together
    back = pd.read_csv('norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'_BH_moredust_case2.ised_ASCII',delimiter='/s', header=None, skiprows = int(len(ages))+7)
    ...
    result.to_csv('norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'_everything_moredust_case2.ised_ASCII',header=None,index=None)
...
def make_ascii(output_number, galaxy_number):
    ...
    os.system('$bc03/ascii_ised norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(galaxy_number)+'_BH_moredust_case2.ised')
...
def make_ised(output_number, galaxy_number):
    ...
    os.system('$bc03/bin_ised norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(galaxy_number)+'_everything_moredust_case2.ised_ASCII')
    os.system('rm -f norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(galaxy_number)+'_BH_moredust_case2.ised_ASCII')
    os.system('rm -f norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(galaxy_number)+'_everything_moredust_case2.ised_ASCII')
...
if output_number == 11:
   redshift = 7
   #read in library file which includes all galaxies with a BH
df = pd.read_csv('norm_catalog/library_galaxies_with_BH'+str(output_number)+'.txt',header=None,
...
for x in galaxy_number:
    ...
    #bh_mass = df_mod['BH mass'].to_numpy()[df_mod['galaxy number'].to_numpy() == x]
    #bh_lum = df_mod['BH Lum'].to_numpy()[df_mod['galaxy number'].to_numpy() == x]


    order_mag_mass = magnitude(bh_mass)#
    print(bh_mass)
    print(order_mag_mass)
    print(bh_lum)
    if order_mag_mass <= 5:
        if bh_lum >36:
          alpha = -1.2
    ...
    if order_mag_mass == 6:
        if bh_lum <= 40:
          alpha = -2
        elif bh_lum < 42 and bh_lum >40:
           alpha = -1.7
        elif bh_lum >42 and bh_lum < 44:
           alpha = -1.2
        elif bh_lum >= 45:
           alpha = -1.2
    if order_mag_mass == 7:
        if bh_lum <= 42:
           alpha = -2
        elif bh_lum >42 and bh_lum <43.5:
           alpha = -1.7
        elif bh_lum >43.5 and bh_lum <45:
            alpha = -1.4
        elif bh_lum >= 45:
           alpha = -1.2
    if order_mag_mass >= 8:
    ...
    cspname = 'norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(x)+'_everything_moredust_case2'
    ...
    outname = 'norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(x)+'_everything_moredust_case2'
