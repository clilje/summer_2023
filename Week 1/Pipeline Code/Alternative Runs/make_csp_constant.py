...
for i in galaxy_number:
    ...
    if df['BH mass'].to_numpy()[0] != 0:
        with open('constant_SFR_catalog/library_galaxies_with_BH'+str(output_number)+'.txt', 'a') as f:
    ...
    with open('constant_SFR_catalog/library_galaxies'+str(output_number)+'.txt', 'a') as f:
        f.write(str(i)+','+str(data_out[-1][0])+','+str(df['Mstar'].to_numpy()[0])+','+str(df['BH mass'].to_numpy()[0])+','+str(df['bolLum2'].to_numpy()[0])+'\n')
    ...
    outname = 'constant_SFR_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)

    #Run the pygalaxev method which returns an .ised and a .mass file
    pygalaxev.run_csp_galaxev(isedname, outname, sfh='constant', sfh_pars=1., tau_V=tau_V,mu=mu, epsilon=0., work_dir=work_dir)
