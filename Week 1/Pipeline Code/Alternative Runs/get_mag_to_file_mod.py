...
for x in df['galaxy number'].to_numpy():
    ...
    with open(work_dir+'magnitudes_galaxies_case1.txt', 'a', newline='') as csvfile:
...
for x in df['galaxy number'].to_numpy():
    ...
    to_print =to_print.to_numpy()
    to_print = np.insert(to_print,0,x)
    with open(work_dir+'magnitudes_galaxies_with_BH'+str(output_number)+'_case5.txt', 'a', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter=',', lineterminator='\n')
        writer.writerow(to_print)