...
def add_spectrum(i, mass_TNG,mass_BC03):
    ...
    age_row = [0]
    #read in row 1
    ages = pd.read_csv('constant_SFR_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'.ised_ASCII',delimiter='/s', header=None, skiprows = lambda x: x not in age_row)
    ...
    ages = ages[1].str.split(expand=True, n=num_ages)
    ...
    specific_rows = [6]
    #read in row 7
    wavelength = pd.read_csv('constant_SFR_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'.ised_ASCII', header=None,delimiter='/s', skiprows = lambda x: x not in specific_rows)
    ...
    df = pd.read_csv('constant_SFR_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'.ised_ASCII',delimiter='/s', header=None, skiprows=7, skipfooter=12)
    ...
    #get first 7 lines
    front = pd.read_csv('constant_SFR_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'.ised_ASCII',delimiter='/s', header=None, skiprows = lambda x: x not in  [0,1,2,3,4,5,6])
    
    ###this section gets the "footer" section of the file in its undisturbed form, to join together
    back = pd.read_csv('constant_SFR_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'.ised_ASCII',delimiter='/s', header=None, skiprows = int(len(ages))+7)
    ...
    result.to_csv('constant_SFR_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'.ised_ASCII',header=None,index=None)
...
def make_ascii(output_number, galaxy_number):
    ...
    os.system('$bc03/ascii_ised constant_SFR_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(galaxy_number)+'.ised')
...
def make_ised(output_number, galaxy_number):
    ...
    os.system('$bc03/bin_ised constant_SFR_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(galaxy_number)+'.ised_ASCII')
    os.system('rm -f constant_SFR_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(galaxy_number)+'.ised_ASCII')
    os.system('rm -f constant_SFR_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(galaxy_number)+'.ised_ASCII')
...
for x in galaxy_number:
    ...
    #print(df_mass)
    #stellar_mass = df_mass[10].to_numpy(float)[-1]
    #print(stellar_mass)
    ...
    cspname = 'constant_SFR_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(x)
    ...
    outname = 'constant_SFR_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(x)
