...
def add_spectrum(i, age_gal_i,bh_mass,bh_lum):
    ...
    result.to_csv('TNG_50_library/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'_mod.ised_ASCII',header=None,index=None)
...
df_mod = pd.read_csv(work_dir+'RescalingBHs_z7_case5.txt',header=None,
...
for x in galaxy_number:
    ...
    bh_mass = df_mod['BH mass'][df_mod['galaxy number'] == x].values[0]
    bh_lum = df_mod['BH Lum'][df_mod['galaxy number'] == x].values[0]
