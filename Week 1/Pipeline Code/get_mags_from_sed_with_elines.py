...
def get_elines(wavelength,Z,sfr,x_id,n_h,log_Us,age):
  #print(Z)
  ...
  for xx in range(len(logUs)):
    if logUs[xx] == round(log_Us*2)/2 and xid[xx] == x_id and nh[xx] == n_h:
        #L (erg/s) = L_gutkin (Lsun / (Msun/yr)) * Lsun (in erg/s) * SFR (in Msun/yr) * (age of the galaxy (in yr) / 1e8 yr)
        #line_flux (from gutking+16)/(line_width (converted from the 300 km/s)*np.sqrt(2*np.pi)))
        line_flux=[line_name[pp][xx]*3.826e+33*sfr/(line_sigma/(2.355*cte_c_kms) * np.sqrt(np.pi*2)) for pp in range(len(line_wavelength))] 
    """
  line_sigma=300 #in km/s ## corresponds to sigma (standard deviation of the gaussian)
  ...
  for ppp in np.arange(len(line_wavelength)):
    ...
    for xx in range(len(logUs)):
        if logUs[xx] == round(log_Us*2)/2 and xid[xx] == x_id and nh[xx] == n_h and rat[xx]==1.0 and m_UV[xx] ==100:
            line_width = line_wavelength[ppp] * line_sigma / (2.355*cte_c_kms)
            line_height = (line_name[ppp][xx]*3.826e33*sfr) / (np.sqrt(2*np.pi)*line_width) 
...
def add_spectrum(i, age_gal_i,Z,sfr,xid,nh,log_Us):
    """
    ...
    specific_rows = [6]
    #read in row 7
    wavelength = pd.read_csv('norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'_lowmass_evenmoredust.ised_ASCII',delimiter='/s', header=None, skiprows = lambda x: x not in specific_rows)
    ...
    elines = get_elines(xvals,Z,sfr,xid,nh,log_Us,age_gal_i)
    ...
    result.to_csv('norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'_lowmass_evenmoredust_elines.ised_ASCII',header=None,index=None)
...
def make_ised(output_number, galaxy_number):
    ...
    os.system('$bc03/bin_ised norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(galaxy_number)+'_lowmass_evenmoredust_elines.ised_ASCII')
...
for x in galaxy_number:
    ...
    add_spectrum(x,age,met_gal,av_sfr,xid,nh,logUs)

    #convert modified .ascii file to .ised for further use
    make_ised(output_number, x)

    #name of modified file
    cspname = 'norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(x)+'_lowmass_evenmoredust_elines'
    ...
    outname = 'norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(x)+'_lowmass_evenmoredust_elines'
