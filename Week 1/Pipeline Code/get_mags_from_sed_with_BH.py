...
def add_spectrum(i, age_gal_i,bh_mass,bh_lum):
    ...
    age_row = [0]
    #read in row 1
    ages = pd.read_csv('norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'_lowmass_evenmoredust_elines.ised_ASCII',delimiter='/s', header=None, skiprows = lambda x: x not in age_row)
    ...
    specific_rows = [6]
    #read in row 7
    wavelength = pd.read_csv('norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'_lowmass_evenmoredust_elines.ised_ASCII',delimiter='/s', header=None, skiprows = lambda x: x not in specific_rows)
    ...
    df = pd.read_csv('norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'_lowmass_evenmoredust_elines.ised_ASCII',delimiter='/s', header=None, skiprows=7, skipfooter=12)
    ...
    #get first 7 lines
    front = pd.read_csv('norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'_lowmass_evenmoredust_elines.ised_ASCII',delimiter='/s', header=None, skiprows = lambda x: x not in  [0,1,2,3,4,5,6])
    
    ###this section gets the "footer" section of the file in its undisturbed form, to join together
    back = pd.read_csv('norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'_lowmass_evenmoredust_elines.ised_ASCII',delimiter='/s', header=None, skiprows = int(len(ages))+7)
    ...
    result.to_csv('norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(i)+'_lowmass_evenmoredust_BH_cont.ised_ASCII',header=None,index=None)
...
def make_ascii(output_number, galaxy_number):
    ...
    os.system('$bc03/ascii_ised norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(galaxy_number)+'_lowmass_evenmoredust_elines.ised')
...
def make_ised(output_number, galaxy_number):
    ...
    #os.system('rm -f TNG_50_library/TNG50_out_'+str(output_number)+'_gal_'+str(galaxy_number)+'_mod.ised_ASCII')
    #os.system('rm -f TNG_50_library/TNG50_out_'+str(output_number)+'_gal_'+str(galaxy_number)+'.ised_ASCII')
    os.system('$bc03/bin_ised norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(galaxy_number)+'_lowmass_evenmoredust_BH_cont.ised_ASCII')
    ...
    os.system('rm -f norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(galaxy_number)+'_lowmass_evenmoredust_elines.ised_ASCII')
...
if output_number == 17:
  ...
  df = pd.read_csv(work_dir+'library_galaxies_with_BH'+str(output_number)+'_lowmass.txt',header=None,
...
#df_mod = pd.read_csv('TNG_50_library/RescalingBHs_case1.txt',header=None,
#                    names=['galaxy number','BH mass','BH Lum'],delimiter='  ')
#df_mod = df_mod[0].str.split(expand=True, n=2)
#print(df_mod)
...
for x in galaxy_number:
    ...
    #bh_mass = bh_mass/10
    #print(df_mod['galaxy number'].to_numpy())
    ...
    cspname = 'norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(x)+'_lowmass_evenmoredust_BH_cont'
    ...
    outname = 'norm_catalog/TNG50_out_'+str(output_number)+'_gal_'+str(x)+'_lowmass_evenmoredust_BH_cont'
